...
namespace taphelper
{
    class SichuanPeopleClub
    {
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine("请点击'答案选项（A,B,C,D)'按钮，只能点击一个");
            var answer = AdbUtils.GetClickCurPoint(cur, eventCur);
            ...
            while(Console.ReadLine() != null)
            {
                using(var process = CmdUtils.createCmd())
                {
                    ...
                    for (var i = 0; i < questionNum; i++)
                    {
                        ...
                        Console.WriteLine(String.Format("第{0}题，已经点击'答案选项（A,B,C,D)'按钮，point:{1}", (i + 1), answer));
                        Thread.Sleep(intervelMillions);
